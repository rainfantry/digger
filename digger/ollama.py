...
##
## This eliminates readline() deadlock.
## ============================================================
...
class OllamaClient:
    ...
    def chat(self, context, stream=True):
        ...
        Args:
            context: Full context string (RAG + conversation)
        ...
        try:
            ...
            ## Check for errors (step 5.5 - stderr capture)
    ...
    def _handle_stderr(self, stderr_output):
        ...
        Filters out spinner/progress ANSI codes that aren't real errors.

        Args:
            stderr_output: Raw stderr string
        """
        ## Filter out ANSI escape sequences (spinner, cursor control)
        ## These are terminal control codes, not errors
        import re
        clean_stderr = re.sub(r'\x1b\[[0-9;?]*[a-zA-Z]', '', stderr_output)
        clean_stderr = re.sub(r'[⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏]', '', clean_stderr)  ## Spinner chars
        clean_stderr = clean_stderr.strip()

        ## If nothing left after cleaning, ignore
        if not clean_stderr:
            return
        ...
        if "model not found" in stderr_lower or "pull" in stderr_lower:
            ...
            ## Unknown error - show it (but only if meaningful)
...
## ============================================================
## QUICK TEST - Run this file directly to test Ollama
## ============================================================
if __name__ == "__main__":
    print("Testing Ollama client...")
    print("=" * 50)

    ## Use mistral by default
